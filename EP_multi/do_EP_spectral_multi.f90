      ...
      ELSE
          ...
          !read in enk, enk+q
          ALLOCATE(enk(numk,norb))
          READ(65,*) ((enk(i,j),j=1,norb),i=1,numk)

          ALLOCATE(enkq(numk,numq,norb))
          READ(65,*) (((enkq(i,j,l),l=1,norb),j=1,numq),i=1,numk)
          ...
          ALLOCATE(Gv(numk,numq,numnv,norb,norb))
          READ(65,*) (((((Gv(i,j,l,m,n),n=1,norb),m=1,norb),l=1,nu),j=1,numq),i=1,numk)
      ...
      ALLOCATE(sigi(numk,numomega,norb))
      ALLOCATE(tot_sigi(numk,numomega,norb))
      ALLOCATE(Green(numk,numomega,norb))
      ALLOCATE(spectral(numk,numomega,norb))
      ...
      DO n = 1, numomega
          ...
          DO ikx = 1, numk
                ...
                DO iqx = 1, numq
                   ...
                   IF (ikwp .GT. 0 .AND. ikwp .LE. numq_per) THEN 
                       ...
                       DO jorb = 1, norb
                          ...
                          epskq = enkq(ikx,iqx,jorb) - mu

                          DO inu = 1,nu

                             !gv(k,q)
                             temp1 = ABS(Gv(ikx,iqx,inu,iorb,jorb))
                             ...
                             IF( wqnv .GT. eps_acustic) THEN 
                              ...
                              sigi(ikx,n,iorb)=sigi(ikx,n,iorb)+weights(iqx)*(temp1**2)*(temp2+temp3)
             ...
             CALL MPI_REDUCE(sigi(ikx,n,iorb),tot_sigi(ikx,n,iorb),1,MPI_DOUBLE_COMPLEX,MPI_SUM,0,MPI_Comm_World,ierr)

             IF (rank .EQ. 0) THEN

               ! calculate epsilon_k
               epsk = enk(ikx,iorb)-mu

               ! calculate the Green function
               Green(ikx,n,iorb) = 1.0/(omega(n)+(0.0,1.0)*delta-epsk-strength*tot_sigi(ikx,n,iorb))

               ! calculate the spectral function
               spectral(ikx,n,iorb)=-AIMAG(Green(ikx,n,iorb))/pi        
            ...
            DO ikx = 1, numk
               DO n = 1, numomega
                  DO iorb = 1, norb
                     ...
                     WRITE(67,'(2F10.6, F24.16)') distance, omega(n), spectral(ikx,n,iorb)
